...
process fastQ_trimming {
    ...
    // Actual trimming using Trimmomatic...
    script:

    """
    trimmomatic PE -phred33 \\
        // Input first fastQ file
        ${fastQ_reads[0]} \\
        // Input second fastQ file
        ${fastQ_reads[1]} \\
        // Output paired reads file 1
        ${sample_id}_r1.paired.fastq.gz \\
        // Output unpaired reads file 1
        ${sample_id}_r1.unpaired.fastq.gz \\
        // Output paired reads file 2
        ${sample_id}_r2.paired.fastq.gz \\
        // Output unpaired reads file 2
        ${sample_id}_r2.unpaired.fastq.gz \\
        // Trimming parameters (sliding window)
        SLIDINGWINDOW:5:30 \\
        // Redirecting stdout to log file
        1> trimmed_stdout.log \\
        // Redirecting error output to log file
